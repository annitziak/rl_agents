    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def schedule_hyperparameters(self, timestep: int, max_timestep: int):
        ...
        ** You may modify the signature of these functions if you wish to pass additional arguments **
        ...
        def epsilon_linear_decay(*args, **kwargs):
            """linear decay epsilon from epsilon_start to epsilon_min over max_timestep timesteps and if
            the current timestep is greater than max_timestep, return epsilon_min"""
            if timestep>= self.exploration_fraction * max_timestep:
                return self.epsilon_min
            else:
                # see if the current timestep is greater than the exploration_fraction * max_timestep
                epsilon = self.epsilon_start - (self.epsilon_start - self.epsilon_min) * (timestep / (self.exploration_fraction * max_timestep))
                return max(epsilon, self.epsilon_min) # precaution to ensure epsilon is not less than epsilon_min


        def epsilon_exponential_decay(*args, **kwargs):
            """exponential decay epsilon from epsilon_start to epsilon_min over max_timestep timesteps and if
            the current timestep is greater than max_timestep, return epsilon_min"""

            if timestep>=max_timestep:
                return self.epsilon_min
            else:
                epsilon = self.epsilon_start * self.epsilon_exponential_decay_factor ** (timestep / max_timestep)
                return max(epsilon, self.epsilon_min) # precaution to ensure epsilon is not less than epsilon_min
    ...
    def act(self, obs: np.ndarray, explore: bool):
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        if explore and np.random.rand() < self.epsilon:
            ...
            # exploit in both cases (explore=False or np.random.rand() >= self.epsilon)
            # can also use torch.argmax(self.critics_net(torch.tensor(obs))).item()
            return self.critics_net(torch.Tensor(obs)).argmax().item()
    ...
    def update(self, batch: Transition) -> Dict[str, float]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q3**
        ...
        #tensors and move to device [to use GPU if available]
        device = next(self.critics_net.parameters()).device  # Get the device
        states = states.clone().detach().to(dtype=torch.float32, device=device)
        next_states = next_states.clone().detach().to(dtype=torch.float32, device=device)
        actions = actions.clone().detach().to(dtype=torch.int64, device=device).view(-1, 1)
        rewards = rewards.clone().detach().to(dtype=torch.float32, device=device).view(-1, 1)
        done = done.clone().detach().to(dtype=torch.float32, device=device).view(-1, 1)
        
        #debugging
        q_values = self.critics_net(states).gather(1, actions) #get the Q-values of the actions taken
        ...
        with torch.no_grad():
            ...
            # compute target Q-values
            target_q_values = rewards + self.gamma * q_values_next.max(dim=1, keepdim=True)[0] * (1 - done)

        # compute the loss between Q-values and target Q-values
        q_loss = torch.nn.functional.mse_loss(q_values, target_q_values)

        # optimize the critics_net
        ...
        #update the counter after each update and before updating the target network
        self.update_counter += 1

        # update the target network
        # hard update of the target network every target_update_freq iterations
...
class DiscreteRL(Agent):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.alpha = kwargs.get("learning_rate", alpha)
        #self.alpha: float = alpha
        print(f"Initialized agent with learning rate: {self.alpha}") # this is essential because otherwise the lr doesnt change!
        ...
        self.position_bins = np.linspace(-1.2, 0.6, 8) #k=8
        # Velocity range: -0.07 to 0.07 (8 bins)
        self.velocity_bins = np.linspace(-0.07, 0.07, 8) #k=8
    ...
    def update(
        ...
        ### PUT YOUR CODE HERE ###
        target_value = reward + self.gamma * (1-done)* max([self.q_table[(next_state, act)] for act in range(self.n_acts)])
