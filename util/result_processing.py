...
# This is some added code to assess performance of the runs
# Description : gets the mean of the final return means of all runs
# gets the standard deviation of the final return means of all runs
# saves the results to a text file
    
def get_mean_rank_runs(runs: List[Run]) -> float:
    """Returns the mean of the final return means of all runs."""
    return np.mean([run.final_return_mean for run in runs])

def get_std_rank_runs(runs: List[Run]) -> float:
    """Returns the standard deviation of the final return means of all runs."""
    return np.std([run.final_return_mean for run in runs], ddof=1)  # Use ddof=1 for sample std dev

def save_ranked_results_to_file(runs: List[Run], filename: str):
    """Saves the ranked results exactly as printed to a text file."""

    os.makedirs(os.path.dirname(filename), exist_ok=True)
    with open(filename, 'w') as f:
        f.write("Ranked runs by final return mean:\n")
        
        for i, run in enumerate(runs, 1):
            f.write(f"{i}. {run.run_name}: Mean Final Return = {run.final_return_mean:.2f}\n")
        
        best_run, best_weights_file = get_best_saved_run(runs)
        f.write(f"\nBest run: {best_run.run_name}, Best saved model: {best_weights_file}\n")

        mean_final = get_mean_rank_runs(runs)
        std_final = get_std_rank_runs(runs)
        
        f.write(f"\nMean rank of all runs: {mean_final:.2f}\n")
        f.write(f"Standard deviation of all runs: {std_final:.2f}\n")
